        ...
        <van-tab title="吊柜">
          ...
          <van-row v-for="(itemW, idxW) in WallCupboard.items" :key="idxW" class="WallCupboardItem SmallSize_14" style="height: 50px;border-top: 0px solid #000;border-bottom: 1px solid #000;">
            ...
            <van-col class="column" span="3">
              <van-field v-model="itemW.height" />
            </van-col>
            <van-col class="column" span="3">
              <van-field v-model="itemW.width" />
            </van-col>
            <van-col class="column" span="3">
              <van-field v-model="itemW.thickness" />
            </van-col>
            <van-col class="column" span="3">
              <van-field v-model="itemW.amount" />
            </van-col>
    ...
    async submitOrder () {
      ...
      if (this.role === '0') {
        ...
        if (WallCupboardItems.length > 0) {
          ...
          // for (let i = 0; i < WallCupboardItems; i++) {
          //   for (let key in WallCupboardItems[i]) {
          //     if (WallCupboardItems[i][key].trim() === '') {
          //       Toast({
          //         duration: 1500,
          //         forbidClick: true,
        ...
        if (FloorCabinetItems.length > 0) {
          ...
          // for (let i = 0; i < FloorCabinetItems; i++) {
          //   for (let key in FloorCabinetItems[i]) {
          //     if (FloorCabinetItems[i][key].trim() === '') {
          //       Toast({
          //         duration: 1500,
          //         forbidClick: true,
          //         message: '请将吊柜每条的信息填写完整!'
